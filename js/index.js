...
var Resultado;
(function (Resultado) {
    Resultado[Resultado["SIMPLES"] = 0] = "SIMPLES";
    Resultado[Resultado["FULL"] = 6] = "FULL";
    Resultado[Resultado["SEGUIDA"] = 7] = "SEGUIDA";
    Resultado[Resultado["QUADRADA"] = 8] = "QUADRADA";
    Resultado[Resultado["GENERAL"] = 9] = "GENERAL";
})(Resultado || (Resultado = {}));
function Full(values) {
    return values.includes(2) && values.includes(3);
}
function Quadrada(values) {
    return values.includes(4);
}
function Seguida(values) {
    return values[0] !== values[5] && values.every((e) => e <= 1);
}
function General(values) {
    return values.includes(5);
}
function Analisador(data) {
    const statistic = {};
    for (let i = 1; i <= 6; i++) {
        statistic[i] = 0;
    }
...
function Jogada(jogador, network, log) {
    sorteio.Sortear();
...
function test(networks) {
    for (let i = 0; i < 25; i++) {
        ...
        //
        networks[5] = Network.Crossover(best_scores[0].Network, best_scores[1].Network);
        for (let i = 6; i < networks.length; i++) {
            const indexA = Math.floor(Math.random() * 5);
            const indexB = Math.floor(Math.random() * 5);
            networks[i] = Network.Crossover(best_scores[indexA].Network, best_scores[indexB].Network);
        }
    }
}
const networks = [];
